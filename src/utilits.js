  ...
  document.querySelectorAll("img.svg").forEach((el) => {
    ...
    fetch(imgURL)
      ...
      .then((response) => {
        ...
        if (!svg) {
          // Fallback to text/html if image/svg+xml doesn't work
        ...
        if (typeof imgID !== "undefined") {
          importedSvg.setAttribute("id", imgID);
        }

        if (typeof imgClass !== "undefined") {
          importedSvg.setAttribute("class", imgClass + " replaced-svg");
        }

        importedSvg.removeAttribute("xmlns:a");
        el.parentNode.replaceChild(importedSvg, el);
      })
      .catch((error) => {
        console.warn(`Failed to load SVG: ${imgURL}`, error);
      });
  });
};
